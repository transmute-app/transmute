...
class CompressionJobDB:
    """Database class for managing queued compression jobs.

    Stores durable status for compressions so users can see queued, running,
    completed, failed, and cancelled jobs even after closing the page or
    restarting the app. Compression is same-format only, so there is no
    output_format column; the source media type is preserved on output.
    """

    settings = get_settings()
    DB_PATH = settings.db_path
    _TABLE_NAME = settings.compression_jobs_table_name
    ...
    def create_tables(self) -> None:
        """Create the compression jobs table if missing and ensure all columns exist."""
        with self.conn:
            self.conn.execute(f"""
    ...
    def insert_job(self, metadata: dict) -> dict:
        """Insert a queued compression job and return the stored row.

        Required keys: ``user_id``, ``source_file_id``.
        Optional keys: ``id``, ``compression_level``, ``compressor_name``,
        ``source_filename``, ``source_media_type``, ``source_extension``,
        ``source_size_bytes``.
        """
        required = {"user_id", "source_file_id"}
        ...
        with self.conn:
            self.conn.execute(
                ...
                INSERT INTO {self.TABLE_NAME} (
                    id, user_id, source_file_id, compression_level,
                    status, compressor_name,
                ...
                (
                    ...
                    metadata.get("compression_level"),
                    STATUS_QUEUED,
                    metadata.get("compressor_name"),
    ...
    def fail_running_jobs(self, error_message: str) -> int:
        ...
        was interrupted mid-compression. Returns the number of rows updated.
        """
        with self.conn:
            cursor = self.conn.execute(
                f"UPDATE {self.TABLE_NAME} SET "  # nosec B608
    ...
    def requeue_running_jobs(self) -> int:
        ...
        Used at startup to recover from crashes/restarts where the worker
        was interrupted mid-compression. Since jobs are not resumable, we
