...
from db import CompressionJobDB
...
def _make_job(user_id="user-a", source_id="file-1", compression_level="balanced", **extra):
    base = {
        "user_id": user_id,
        "source_file_id": source_id,
        "compression_level": compression_level,
        "compressor_name": "PillowCompress",
...
def test_table_has_all_expected_columns(job_db):
    ...
    expected = {
        ...
        "compressor_name", "source_filename", "source_media_type",
...
def test_insert_job_returns_queued_row(job_db):
    ...
    assert job["compression_level"] == "balanced"
...
def test_insert_job_rejects_missing_required_fields(job_db):
    with pytest.raises(ValueError):
        job_db.insert_job({"compression_level": "max"})
