"""Background worker that processes queued compression jobs.

The worker runs in a daemon thread, claims one job at a time from
``CompressionJobDB`` using an atomic ``BEGIN IMMEDIATE`` transaction, executes
it via ``services.run_compression_job``, and records the terminal status.

Design notes:
- Single-process, single-worker by default. Concurrency higher than 1 is
    configurable in settings but not exercised in v1.
- Errors during compression always mark the job ``failed``; they never
    crash the worker thread.
- On startup, ``recover_compression_jobs`` is called to move any jobs left in
...
def _process_compression_job(
    ...
    *,
    ...
    default_compression_levels_db: DefaultCompressionLevelsDB,
    job_db: CompressionJobDB,
) -> None:
    ...
    if compressor_type is None:
        job_db.mark_failed(
            job_id,
            f"No compressor found for {media_format}",
        )
        return
    ...
    job_db.mark_completed(job_id, output_file_id=compressed_metadata["id"])
    logger.info(
        "Job %s completed: %s compressed (output %s)",
        job_id, media_format, compressed_metadata["id"],
    )


def recover_compression_jobs() -> int:
    ...
    if affected:
        logger.warning("Re-queued %d stale running compression job(s) on startup", affected)
    return affected


def compression_worker_task(stop_event: threading.Event | None = None) -> None:
...
def compression_worker_manager_task(
    stop_event: threading.Event | None = None,
    worker_concurrency: int | None = None,
) -> None:
    """Spawn compression workers lazily up to the configured concurrency.
    ...
    max_workers = max(1, worker_concurrency or settings.compression_worker_concurrency)
    job_db = CompressionJobDB()
    workers: list[dict] = []

    logger.info("Compression queue manager started (max workers=%d)", max_workers)
    while True:
        if stop_event is not None and stop_event.is_set():
        ...
        for _ in range(additional_workers):
            ...
            worker = get_compression_worker_thread(stop_event=worker_stop_event)
            worker.start()
...
def get_compression_worker_thread(stop_event: threading.Event | None = None) -> threading.Thread:
    """Return a daemon thread targeting ``compression_worker_task``.
    ...
    blocked waiting on a long-running compressor.
    """
    thread = threading.Thread(
        target=compression_worker_task,
        args=(stop_event,),
        name="compression-queue-worker",
        daemon=True,
    )
    return thread


def get_compression_worker_manager_thread(
    stop_event: threading.Event | None = None,
    worker_concurrency: int | None = None,
) -> threading.Thread:
    """Return a daemon thread targeting ``compression_worker_manager_task``."""
    thread = threading.Thread(
        target=compression_worker_manager_task,
        args=(stop_event, worker_concurrency),
        name="compression-queue-manager",
        daemon=True,
    )
    return thread
